...
if ( apply_filters( 'ylc_prefill_login_logged_user', false ) && is_user_logged_in() ) {
...
<div id="YLC">
    ...
    <div id="YLC_chat_btn" class="chat-chat-btn btn-<?php echo $args['button_type']; ?>">
        ...
        <div class="chat-ico ylc-toggle fa fa-angle-<?php echo( $args['button_pos'] == 'bottom' ? 'up' : 'down' ) ?>"></div>
        <div class="chat-title">
			<?php echo ylc_sanitize_text( YITH_Live_Chat()->options['text-chat-title'] ) ?>
        </div>
    </div>
    ...
    <div id="YLC_chat" class="chat-widget">
        ...
        <div id="YLC_chat_header" class="chat-header">
            <div class="chat-ico chat fa fa-comments"></div>
            <div class="chat-ico ylc-toggle fa fa-angle-<?php echo( $args['button_pos'] == 'bottom' ? 'down' : 'up' ) ?>"></div>
            <div class="chat-title">
				<?php echo ylc_sanitize_text( YITH_Live_Chat()->options['text-chat-title'] ) ?>
            </div>
        ...
        <div id="YLC_chat_body" class="chat-body chat-online" style="<?php echo $args['chat_width'] ?>">
            <div class="chat-cnv" id="YLC_cnv">
                <div class="chat-welc">
					<?php echo ylc_sanitize_text( YITH_Live_Chat()->options['text-start-chat'], true ) ?>
                </div>
            </div>
        ...
        <div id="YLC_offline" class="chat-body chat-form" style="<?php echo $args['form_width'] ?>">
            <div class="chat-lead op-offline">
				<?php echo ylc_sanitize_text( YITH_Live_Chat()->options['text-offline'], true ) ?>
            </div>
            <div class="chat-lead op-busy">
				<?php echo ylc_sanitize_text( YITH_Live_Chat()->options['text-busy'], true ) ?>
            </div>
        ...
        <div id="YLC_login" class="chat-body chat-form" style="<?php echo $args['form_width'] ?>">
            <div class="chat-lead">
				<?php echo ylc_sanitize_text( YITH_Live_Chat()->options['text-welcome'], true ) ?>
            </div>
            <form id="YLC_login_form" action="">
                ...
                <div class="form-line">
                    <input type="text" name="user_name" id="YLC_field_name" placeholder="<?php _e( 'Please enter your name', 'yith-live-chat' ) ?>" value="<?php echo $username ?>">
                ...
                <div class="form-line">
                    <input type="email" name="user_email" id="YLC_field_email" placeholder="<?php _e( 'Please enter your email', 'yith-live-chat' ) ?>" value="<?php echo $usermail ?>">
                    <i class="chat-ico fa fa-envelope-o"></i>
                </div>
				<?php if ( YITH_Live_Chat()->options['chat-gdpr-compliance'] == 'yes' && defined( 'YLC_PREMIUM' ) && YLC_PREMIUM ): ?>
                    <div class="form-line">
                        <div class="chat-checkbox">
                            <input type="checkbox" id="YLC_chat_gdpr_acceptance">
                            <label for="YLC_chat_gdpr_acceptance">
								<?php echo ylc_sanitize_text( YITH_Live_Chat()->options['chat-gdpr-checkbox-label'] ) ?>
                            </label>
                        </div>
                        <br />
						<?php

						$privacy_page = YITH_Live_Chat()->options['offline-gdpr-privacy-page'];

						if ( $privacy_page == '' ) {
							$privacy_page = get_permalink( get_option( 'wp_page_for_privacy_policy' ) );
						}

						$gdpr_text = ylc_sanitize_text( YITH_Live_Chat()->options['chat-gdpr-checkbox-desc'], true );
        ...
        <div id="YLC_end_chat" class="chat-body chat-form" style="<?php echo $args['form_width'] ?>">
            <div class="chat-lead">
				<?php echo ylc_sanitize_text( YITH_Live_Chat()->options['text-close'], true ); ?>
            </div>
			<?php if ( defined( 'YLC_PREMIUM' ) && YLC_PREMIUM ) : ?>
